...
تنبيه هام: إذا كان النص طويلاً، يجب إعادة صياغته بالكامل مع الحفاظ على كافة التفاصيل والمعلومات المهمة وعدم اختصاره أبداً إلا إذا طلبت القواعد ذلك صراحة.
  ...
  async applyFilters(
    ...
    if (aiFilters?.isEnabled) {
      ...
      if (rulesArray.length > 0) {
        ...
        const prompt = `أنت خبير في تنفيذ تعليمات الفلترة الصارمة. مهمتك هي تقييم الرسالة بناءً على القواعد المحددة "فقط".

**معلومات عن الرسالة:**
- عدد كلمات الرسالة: ${wordCount} كلمة.

**تحذير هام جداً:**
1. لا تستخدم حكمك الشخصي أو الأخلاقي أو السياسي خارج نطاق القواعد المكتوبة.
2. "هام جداً": الأخبار التي تتعلق بمحافظات يمنية مثل (حضرموت، المهره، سقطرى، عدن، صنعاء، إلخ) هي "شأن يمني داخلي" ولا يجب حظرها تحت قاعدة "شأن غير يمني".
3. إذا لم تخالف الرسالة أياً من القواعد المذكورة أدناه بشكل صريح، فيجب أن يكون القرار "ALLOW".
4. في وضع "Blacklist": اسمح بكل شيء (ALLOW) "إلا" إذا وجد تعارض صريح مع إحدى القواعد.
5. في وضع "Whitelist": امنع كل شيء (BLOCK) "إلا" إذا كانت الرسالة تطابق إحدى القواعد صراحة.
6. "هام جداً": عند تقييم قاعدة طول النص (مثلاً 100 كلمة)، اعتمد على "عدد كلمات الرسالة" المذكور أعلاه (${wordCount} كلمة) ولا تحسب كلمات هذا البرومبت أو القواعد.

الرسالة المراد تحليلها: "${textToAnalyze}"

القواعد المطلوبة للتنفيذ:
${rulesDescription}
